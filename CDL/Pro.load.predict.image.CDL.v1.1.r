...
# rm(list=ls(all=TRUE)); samplerate <- 0.1; con_layer <- 2; is_shallow=TRUE ; learningrate <- 0.01; is_norm <- TRUE;  beta.coe<-1E-3;    iters <- 32000;  source("Pro.load.predict.image.CDL.v1.1.r")

# 4-layer 0.9 sample rate
# rm(list=ls(all=TRUE)); samplerate <- 0.9; con_layer <- 2; is_shallow=TRUE ; learningrate <- 0.01; is_norm <- TRUE;  beta.coe<-1E-3;    iters <- 300000; source("Pro.load.predict.image.CDL.v1.1.r")
...
# save (classes, file="parameters.rfile.data")        
...
CNN_TIF=paste0("/gpfs/data2/workspace/zhangh/ard.Mar01.2018/cdl.samplerate",samplerate,".is_shallow",is_shallow,"/")
