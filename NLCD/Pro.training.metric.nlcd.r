# rm(list=ls(all=TRUE)); samplerate <- 0.5;  con_layer <- 2; is_shallow=TRUE ; learningrate <- 0.01; is_norm <- FALSE; beta.coe<-1E-3;   iters <- 900000; source("Pro.training.metric.nlcd.r")

# test only 
# rm(list=ls(all=TRUE)); samplerate <- 0.1;  con_layer <- 2; is_shallow=TRUE ; learningrate <- 0.01; is_norm <- TRUE; beta.coe<-1E-3;   iters <- 1800; source("Pro.training.metric.nlcd.r")

## 0.1
# rm(list=ls(all=TRUE)); samplerate <- 0.1;  con_layer <- 2; is_shallow=TRUE ; learningrate <- 0.01; is_norm <- TRUE; beta.coe<-1E-3;   iters <- 180000; source("Pro.training.metric.nlcd.r")
# rm(list=ls(all=TRUE)); samplerate <- 0.1;  con_layer <- 2; is_shallow=FALSE; learningrate <- 0.01; is_norm <- TRUE; beta.coe<-1E-3;   iters <- 180000; source("Pro.training.metric.nlcd.r")
# rm(list=ls(all=TRUE)); samplerate <- 0.1;  con_layer <- 2; is_shallow=5    ; learningrate <- 0.01; is_norm <- TRUE; beta.coe<-1E-3;   iters <- 180000; source("Pro.training.metric.nlcd.r")
# rm(list=ls(all=TRUE)); samplerate <- 0.1;  con_layer <- 2; is_shallow=10   ; learningrate <- 0.01; is_norm <- TRUE; beta.coe<-1E-3;   iters <- 180000; source("Pro.training.metric.nlcd.r")
                                             
## 0.25                                      
# rm(list=ls(all=TRUE)); samplerate <- 0.25; con_layer <- 2; is_shallow=TRUE ; learningrate <- 0.01; is_norm <- TRUE; beta.coe<-1E-3;   iters <- 460000; source("Pro.training.metric.nlcd.r")
# rm(list=ls(all=TRUE)); samplerate <- 0.25; con_layer <- 2; is_shallow=FALSE; learningrate <- 0.01; is_norm <- TRUE; beta.coe<-1E-3;   iters <- 460000; source("Pro.training.metric.nlcd.r")
# rm(list=ls(all=TRUE)); samplerate <- 0.25; con_layer <- 2; is_shallow=5    ; learningrate <- 0.01; is_norm <- TRUE; beta.coe<-1E-3;   iters <- 460000; source("Pro.training.metric.nlcd.r")
                                             
## 0.5                                       
# rm(list=ls(all=TRUE)); samplerate <- 0.5;  con_layer <- 2; is_shallow=TRUE ; learningrate <- 0.01; is_norm <- TRUE; beta.coe<-1E-3;   iters <- 900000; source("Pro.training.metric.nlcd.r")
# rm(list=ls(all=TRUE)); samplerate <- 0.5;  con_layer <- 2; is_shallow=FALSE; learningrate <- 0.01; is_norm <- TRUE; beta.coe<-1E-3;   iters <- 900000; source("Pro.training.metric.nlcd.r")
# rm(list=ls(all=TRUE)); samplerate <- 0.5;  con_layer <- 2; is_shallow=5    ; learningrate <- 0.01; is_norm <- TRUE; beta.coe<-1E-3;   iters <- 900000; source("Pro.training.metric.nlcd.r")
# rm(list=ls(all=TRUE)); samplerate <- 0.5;  con_layer <- 2; is_shallow=10   ; learningrate <- 0.01; is_norm <- TRUE; beta.coe<-1E-3;   iters <- 900000; source("Pro.training.metric.nlcd.r")
                                             
## 0.75                                      
# rm(list=ls(all=TRUE)); samplerate <- 0.75; con_layer <- 2; is_shallow=TRUE ; learningrate <- 0.01; is_norm <- TRUE; beta.coe<-1E-3;   iters <- 1300000; source("Pro.training.metric.nlcd.r")
# rm(list=ls(all=TRUE)); samplerate <- 0.75; con_layer <- 2; is_shallow=FALSE; learningrate <- 0.01; is_norm <- TRUE; beta.coe<-1E-3;   iters <- 1300000; source("Pro.training.metric.nlcd.r")
# rm(list=ls(all=TRUE)); samplerate <- 0.75; con_layer <- 2; is_shallow=5    ; learningrate <- 0.01; is_norm <- TRUE; beta.coe<-1E-3;   iters <- 1300000; source("Pro.training.metric.nlcd.r")
                                             
## 0.9                                       
# rm(list=ls(all=TRUE)); samplerate <- 0.9;  con_layer <- 2; is_shallow=TRUE ; learningrate <- 0.01; is_norm <- TRUE; beta.coe<-1E-3;   iters <- 1600000; source("Pro.training.metric.nlcd.r")
# rm(list=ls(all=TRUE)); samplerate <- 0.9;  con_layer <- 2; is_shallow=FALSE; learningrate <- 0.01; is_norm <- TRUE; beta.coe<-1E-3;   iters <- 1600000; source("Pro.training.metric.nlcd.r")
# rm(list=ls(all=TRUE)); samplerate <- 0.9;  con_layer <- 2; is_shallow=5    ; learningrate <- 0.01; is_norm <- TRUE; beta.coe<-1E-3;   iters <- 1600000; source("Pro.training.metric.nlcd.r")
# rm(list=ls(all=TRUE)); samplerate <- 0.9;  con_layer <- 2; is_shallow=10   ; learningrate <- 0.01; is_norm <- TRUE; beta.coe<-1E-3;   iters <- 1600000; source("Pro.training.metric.nlcd.r")
...
## **************************************************
## find classes 1 & 5
classes_nlcd <- read.csv("./nlcd.legend.csv", header=TRUE);
...
if (is_shallow==10) {
	...
	for (i in 1:itern)  {
		...
		ptm1 <- proc.time();	## ris.rf2 <- randomForest(x=xtrain, y=yftrain, xtest=xtest, ytest=yftext, replace=FALSE, ntree=ntrees, keep.forest=TRUE); # FAILED 
        if (samplerate != 0.9) {
            cat("...normal Random forest when samplerate != 0.9 ....\n");
            ## only training
        ...
        } else { ## when samplerate == 0.9 the above training failed
            ## only training
            cat("...weird Random forest when samplerate != 0.9 ....\n");
            ris.rf2 <- randomForest(x=xtrain, y=yftrain,  xtest=xtest[1,], ytest=yftext[1], replace=FALSE, ntree=ntrees); ## keep.forest cannot be set TRUE
            train.time.i <- proc.time() - ptm1;
            cat(paste("\n", sprintf("%.3f",train.time.i[1]/3600),"\t", sprintf("%.3f",train.time.i[2]/3600),"\t", sprintf("%.3f",train.time.i[3]/3600),"hours\n"));
            ## training and testing
            ptm2 <- proc.time();
            result <- rf.my(xtrain, ytrain, xtest, ytest);
            test.time.i <- proc.time() - ptm2 - train.time.i;
            cat(paste("\n", sprintf("%.3f",test.time.i[1]/3600),"\t", sprintf("%.3f",test.time.i[2]/3600),"\t", sprintf("%.3f",test.time.i[3]/3600),"hours\n"));
        }
        # print (result)
...
print_sta(ptm_sub, conf, itern, accuracies, kapps, classn);
## class_names added on May 1st but not systematically run 
save(samplerate, class_names, split.samples, accuracies, kapps, users, producers, train.time, test.time, 
	file=paste0(".//itern", itern, "samplerate", samplerate, "is_shallow",  is_shallow, "nlcd") );
